...
export default function ArticleUpdateFormContainer({articleId, title = "", content = ""}: ArticleUpdateFormContainerParams) {
    ...
    const handleUpdateArticle = async () => {
        if (articleId !== undefined) {
            // TODO 게시글 수정 API 연동
            console.log(`Update Article, articleId: ${articleId}, title: ${titleValue}, content: ${contentValue}`);
        } else {
            await createArticle({
                title: titleValue,
        ...
        <div className="flex justify-center items-center flex-col gap-3 px-24 py-10">
            <div className="flex flex-col gap-3 h-full" style={{width: "50%"}}>
                <BasicCard title={(articleId === undefined) ? '신규 게시글' : '게시글 수정'}>
                    <TextInput labelName="제목" onChange={handleTitleValue} content={titleValue} placeholder="제목을 입력해 주세요."></TextInput>
